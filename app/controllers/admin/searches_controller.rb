class Admin::SearchesController < Admin::ApplicationController
  ...
  # 各モデルに応じた検索メソッド
  def search_users
    search_model_optimized(User.order(created_at: :desc), %w[nickname bio]).page(params[:user_page])
  end
  
  def search_user_posts
    search_model_optimized(UserPost.order(created_at: :desc), %w[title body]).page(params[:user_post_page])
  end

  def search_groups
    search_model_optimized(Group.order(created_at: :desc), %w[name description]).page(params[:group_page])
  end

  def search_group_posts
    search_model_optimized(GroupPost.order(created_at: :desc), %w[title body]).page(params[:group_post_page])
  end
