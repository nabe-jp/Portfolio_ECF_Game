class Public::RegistrationsController < Devise::RegistrationsController
  ...
  def handle_password_update
    ...
    errors = update_password_for_user(resource, params)
    if errors.any?
  ...
  def handle_profile_update
    if resource.update_without_password(account_update_params.except(:current_password, :password, :password_confirmation))
      redirect_to after_update_path_for(resource), notice: "プロフィールを更新しました"
    else
      store_form_data(attributes: account_update_params, error_keys: [:last_name, :first_name, :email], form_type: :profile)
