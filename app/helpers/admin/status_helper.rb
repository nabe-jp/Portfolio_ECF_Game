module Admin::StatusHelper
  def status_label(record)
    ...
    if record.is_a?(User) && record.respond_to?(:user_status)
      return content_tag(:span, "有効", class: "badge bg-secondary") if record.user_status == "active"
      return content_tag(:span, "無効", class: "badge bg-success")
    end
    ...
    # 固定
    if !record.is_a?(User) && record.respond_to?(:is_pinned) && record.is_pinned
      return content_tag(:span, "固定", class: "badge bg-warning text-dark")
    end
  
    # 公開 / 非公開
    if !record.is_a?(User) && record.respond_to?(:is_public)
      return content_tag(:span, "公開", class: "badge bg-success") if 
  ...
  def row_class(record)
    return "table-secondary" if !record.is_a?(User) && record.respond_to?(:is_deleted) && record.is_deleted
    return "" # 通常行
  end
end