module Restorer
  class GroupEventRestorer
    def initialize(event, restored_due_to_parent: false)
      @event = event
    ...
    def call
      ActiveRecord::Base.transaction do
        ...
        # 親の連鎖削除で削除されたものを復元
        if @restored_due_to_parent && @event.deleted_due_to_parent
          restore_params[:hidden_on_parent_restore] = true

        # 個別の復元
        elsif !@restored_due_to_parent && !@event.deleted_due_to_parent
